...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/test_runs")
    return parser.parse_args()

def run_single(args, fold, run_id, prompt):
    cmd = ["python", "scripts/utils/test_single_run.py", 
        ...
        "--prompt", prompt,
        "--run_id", str(run_id),
        "--output_dir", f"{args.output_dir}/runs/{args.dataset}"]
...
def aggregate(args, fold):
    csv_files = list((Path(args.output_dir) / "runs" / f"{args.dataset}").glob("run_*.csv"))
    if not csv_files:
        return None, None
    
    df = pd.concat([pd.read_csv(f) for f in csv_files], ignore_index=True)
    exclude_cols = ['run', 'model', 'dataset', 'fold', 'prompt']
    aggr_cols = [col for col in df.columns if col not in exclude_cols]
    summary = df.groupby('prompt')[aggr_cols].agg(['mean', 'std']).round(4)
...
def main():
    ...
    output_file = outdir / f"{args.dataset}.csv"
    ...
    for fold in range(1, n_folds + 1):
        ...
        runs_dir = Path(args.output_dir) / "runs" / f"{args.dataset}"
